...
| -------------------------------------------------------------------- | --------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Add or remove a charging station                                     | scenario JSON only                                                                      | `Route.nodes`, `route.segments`, `stations` all live in JSON. Solver iterates `scenario.stations`, no hardcoded names.                                                                                                                            |
...
| Priority buses (express services that should jump the queue)         | New `priority` field on `Bus`; new soft term                                            | Same logic as the peak-price example above.                                                                                                                                                                                                       |
...
| Priority service guarantee (bus must never wait more than N minutes) | Add `priority_pass_wait_time` to `Bus`; one hard constraint in `solver.py`              | Reuses the existing `bus_total_wait` variable. Example: `model.Add(bus_total_wait[bus.id] <= 15)`. High-priority services receive a guaranteed maximum waiting time without affecting the rest of the model structure.                            |
| Safety battery reserve / minimum SoC requirement                     | Add `min_soc_limit` to `Bus`; update battery-range covering constraint in `solver.py`   | Compute an effective range per bus (`R_effective = R × (100 - min_soc_limit)/100`) and use it in the range-cover inequalities. This enforces a mandatory battery reserve while preserving the current high-performance combinatorial formulation. |
